...
fun setQRv2(context: Context, code: String?): Bitmap? {
    if (code.isNullOrEmpty()) return null

    val manager = context.getSystemService(Context.WINDOW_SERVICE) as WindowManager
    val display = manager.defaultDisplay

    // Create a Point to get screen size
    val point = Point()
    display.getSize(point)
    val width = point.x
    val height = point.y

    // Set the dimension for the QR code based on the screen size
    var dimen = if (width < height) width else height
    dimen *= 3 / 4 // Optional: Make the QR code slightly smaller than full width

    // Use ZXing BarcodeEncoder to create the QR code
    return try {
        val barcodeEncoder = BarcodeEncoder()
        val bitMatrix: BitMatrix = barcodeEncoder.encode(code, BarcodeFormat.QR_CODE, dimen, dimen)
