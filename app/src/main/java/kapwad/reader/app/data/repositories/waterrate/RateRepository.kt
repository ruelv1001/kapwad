...
class RateRepository @Inject constructor(
    ...
    fun getAllWRRe(): Flow<List<RateReListModelData>> {
        return flow {
            val response = rateRemoteDataSource.getRateReList()
            emit(response)
        }.flowOn(ioDispatcher)
    }
    //---------------------------------------
    ...
    fun createRe(rate: List<RateReListModelData>): Flow<List<RateReListModelData>> {
        return flow {
            rateLocalDataSource.insertWRRe(rate)
            emit(rate)
        }.flowOn(ioDispatcher)
    }
    ...
    fun getRateRe(): Flow<List<RateReListModelData>> {
        return flow {
            val response = rateLocalDataSource.getWRRe()
            emit(response)
        }.flowOn(ioDispatcher)
    }
    ...
    fun getRateCById(id: String): Flow<RateCListModelData?> {
        return flow {
            val response = rateLocalDataSource.getRateCDetailsById(id)
            emit(response)
        }.flowOn(ioDispatcher)
    }

    fun getRateBById(id: String): Flow<RateBListModelData?> {
        return flow {
            val response = rateLocalDataSource.getRateBDetailsById(id)
            emit(response)
        }.flowOn(ioDispatcher)
    }

    fun getRateAById(id: String): Flow<RateAListModelData?> {
        return flow {
            val response = rateLocalDataSource.getRateADetailsById(id)
            emit(response)
        }.flowOn(ioDispatcher)
    }

    fun getRateById(id: String): Flow<RateListModelData?> {
        return flow {
            val response = rateLocalDataSource.getRateDetailsById(id)
            emit(response)
        }.flowOn(ioDispatcher)
    }

    fun getRateReById(id: String): Flow<RateReListModelData?> {
        return flow {
            val response = rateLocalDataSource.getRateReDetailsById(id)
            emit(response)
        }.flowOn(ioDispatcher)
    }
}